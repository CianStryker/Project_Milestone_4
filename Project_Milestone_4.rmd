...
ggplot(China_simple_1, aes(xdadf = year, y = quantity_in_millions, fill = year)) +
  guides(fill = FALSE) +
  geom_col() +
  labs(
...
```{r Commulative Regional Investment}
China_1x <- China_1 %>%
...
ggplot(China_sector3, aes(x = sector, y = y_cum2, fill = region)) +
  geom_col() +
  labs(x = "FDI", y = "Sector", fill=NULL, title="2019") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 16)) +
  guides(fill = FALSE) +
  facet_wrap(~region)
